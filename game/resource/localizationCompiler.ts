...
export class LocalizationCompiler
{
    ...
    GenerateContentStringForLanguage(language: string, localized_data: LocalizationData): string
    {
        ...
        if (localized_data.WeaponsArray) {
            for (const weaponLocalization of localized_data.WeaponsArray)
            {
                let className = weaponLocalization.class_name;

                // Check for name override for the language we're checking
                let weapon_name = weaponLocalization.name;
                let weapon_description = weaponLocalization.description;

                if (weaponLocalization.language_overrides && weaponLocalization.language_overrides.length > 0)
                {
                    for (const language_override of weaponLocalization.language_overrides)
                    {
                        if (language_override.language === language)
                        {
                            if (language_override.name_override)
