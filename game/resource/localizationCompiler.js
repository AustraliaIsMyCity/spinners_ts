...
var LocalizationCompiler = /** @class */ (function () {
    ...
    LocalizationCompiler.prototype.GenerateContentStringForLanguage = function (language, localized_data) {
        ...
        // Go over weapons
        if (localized_data.WeaponsArray) {
            for (var _w = 0, _x = localized_data.WeaponsArray; _w < _x.length; _w++) {
                var weaponLocalization = _x[_w];
                var className = weaponLocalization.class_name;
                // Check for name override for the language we're checking
                var weapon_name = weaponLocalization.name;
                var weapon_description = weaponLocalization.description;
                if (weaponLocalization.language_overrides && weaponLocalization.language_overrides.length > 0) {
                    for (var _y = 0, _z = weaponLocalization.language_overrides; _y < _z.length; _y++) {
                        var language_override = _z[_y];
                        if (language_override.language === language) {
                            if (language_override.name_override) {
                                weapon_name = language_override.name_override;
                            }
                            if (language_override.description) {
                                weapon_description = language_override.description;
                            }
                        }
                    }
                }
                localization_content += "\t\t\"" + className + "_name\" \"" + weapon_name + "\"";
                localization_content += "\n";
                localization_content += "\t\t\"" + className + "_description\" \"" + weapon_description + "\"";
                localization_content += "\n";
            }
        }
